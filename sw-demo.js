...
// 1
  ...
  //   evt.respondWith(
  //     caches.open(StaticAssets).then(function(cache) {
  //       return fetch(evt.request).then(function(response) {
  //         cache.put(evt.request, response.clone());
  //         return response;
  //       });
  //     }).catch(function(e){
  //         return console.warn('Couldn\'t serve response for "%s" from cache: %O', evt.request.url, e), fetch(evt.request)
  //     })
  //   );
  // });
...
// SERVICE WORKER COOKBOOK A MESS
...
// 5 https://pwafire.org/developer/pwa/started/#sw-config-for-node
...
// 6 https://adactio.com
    ...
    //                     .then( responseFromCache => {
    //                         resolveWithResponse(
    //                             responseFromCache || caches.match('/fallback.html')
    //                         );
    //                     });
    //                 });
    ...
    //                 if (request.url.match(/\.(jpg|png|gif|svg|mapbox)/)) {
    //                     const copy = responseFromFetch.clone();
    //                     try {
    //                         event.waitUntil(
